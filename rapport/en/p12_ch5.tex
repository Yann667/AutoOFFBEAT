...
This chapter reports the obstacles I ran into. I have kept only the ones that
changed a design decision or the way I worked; the rest were just lost time.
...
actually in use. I spent a long time trying to update a driver that was already up to date,
because the message said so explicitly. It had not occurred to me to ask which
...
configuration, even though the fast one was in place. I had been measuring for two days without suspecting that my figures
described a service other than the one I thought I was addressing. I now check
...
I lost half a day before thinking to check whether the address itself was
still valid. Absent documentation would have sent me straight to looking for the
public repository; the wrong address kept me searching in the right place for
...
machine. This kind of defect only shows up during a transfer. Had I moved the project
to a second machine earlier, I would have found them at a point where fixing the
...
What I take from this is that I had written the word ``conservative'' in a
...
I draw two things from this. First, I had read my cross-validation score as
evidence that the model was good, when it only told me the model fitted the data
I had given it. I now try to read a metric together with the domain it was
...
re-examining the decision it had motivated. I had not gone back over a
...
is a closed gap reported as safe? What I could not delegate was
knowing what the number ought to look like, and I expect the same limit applies
to the agent I built.
