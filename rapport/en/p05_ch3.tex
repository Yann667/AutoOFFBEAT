...
It is worth clarifying first what is meant by a ``digital twin of a power
plant''. A complete digital twin of a plant would handle the full coupled
...
and conversely. OFFBEAT, however, only deals with fuel performance. What is
built here is therefore the digital twin of a \textbf{single fuel rod}, and
everything outside that domain is treated as a boundary condition.
...
criterion, a status: safe, close to the threshold, or exceeded. It mirrors
exactly the pattern of self-healing — an editable JSON knowledge base, a
...
direction — must the quantity stay below or above the threshold? This last point
...
It must be stressed that the thresholds currently written in the base are
\textbf{indicative values}, explicitly flagged as unvalidated. They are
...
For each non-green criterion, the time evolution of the peak is extracted, a
linear trend is fitted, and the instant at which the threshold is crossed is
extrapolated.
...
on a simulation that has already been run. Exploring a new scenario — what
happens if the power increases by ten per cent? — requires re-running the
solver.
...
Validation is carried out by \textit{leave-one-out}: each point is predicted by
