...
\addplot[color=black!55,mark=triangle*,dashed] coordinates {
  (170.0,623.3) (354.1,686.2) (456.9,702.0)
  (590.2,747.0) (710.9,787.8) (729.2,793.8)};
\addlegendentry{$\Delta T$ across the pellet, fixed $z$}
\end{axis}
\end{tikzpicture}
\caption[Pellet and cladding temperatures over the cycle]{Evolution of the
temperatures over the cycle. The two upper curves are maxima taken over the
whole rod; the dashed curve is the centre-to-edge drop across the pellet at a
fixed axial position, \SI{0.45}{\metre}, where the local power stays constant
to within \SI{0.5}{\percent}.}
\label{fig:temp}
\end{figure}

The cladding temperature remains remarkably stable, around \SI{637}{\kelvin},
which is consistent : it is essentially imposed by the coolant.

The pellet curve is less straightforward, and reading it taught us something.
It does not rise monotonically : it peaks at \SI{1489}{\kelvin} on day
\num{457}, falls back to \SI{1444}{\kelvin}, then rises again. Degradation of
the fuel conductivity cannot explain that, since it only ever goes one way.

The cause is that this curve is a maximum taken over the \emph{whole} rod,
while the axial power profile of the template shifts between three tabulated
shapes over the cycle. The location of the hottest point moves accordingly,
from \SI{0.75}{\metre} to \SI{0.45}{\metre}, then \SI{1.05}{\metre}, and the
local power density there varies by \SI{10}{\percent}. The peak of day
\num{457} is simply the moment when the axial peaking factor is highest.

Fixing the axial position separates the two effects. At \SI{0.45}{\metre},
where the local power stays constant to within \SI{0.5}{\percent}, the drop
across the pellet rises monotonically from \SI{623}{\kelvin} to
\SI{794}{\kelvin}, that is a \SI{27}{\percent} increase in the thermal
resistance of the fuel. \textbf{That} is the degradation of the conductivity
under irradiation, and it is monotonic as it should be.
